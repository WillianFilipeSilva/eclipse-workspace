...
@WebServlet(urlPatterns = { "/login", "/cadastro-usuario", "/alterar-senha", "/definir-senha", "/inserir-usuario",
		"/atualizar-usuario", "/excluir-usuario", "/logar-usuario", "/exibir-perfil" })
public class UsuarioServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest requisicao, HttpServletResponse resposta)
		...
		try {
			...
			case "/logar-usuario":
				logarUsuario(requisicao, resposta);
				break;
	...
	private void logarUsuario(HttpServletRequest requisicao, HttpServletResponse resposta)
			throws ServletException, IOException {

		String email = requisicao.getParameter("email");
		String senha = requisicao.getParameter("senha");

		Usuario usuario = validarCredenciais(email, senha);

		if (usuario != null) {
			HttpSession sessao = requisicao.getSession();
			sessao.setAttribute("usuarioLogado", usuario);

			resposta.sendRedirect("feed-pessoal.jsp");
		} else {
	...
	private Usuario validarCredenciais(String email, String senha) {
		...
		Usuario usuario = usuarioDAO.consultarUsuarioEmail(email);
