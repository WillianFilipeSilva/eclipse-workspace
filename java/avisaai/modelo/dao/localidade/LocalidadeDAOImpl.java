...
public class LocalidadeDAOImpl implements LocalidadeDAO {
	...
	public Localidade consultarLocalidade(String estado, String cidade, String bairro, String logradouro,
			short numero) {
		...
		try {
			...
			List<Localidade> localidades = consultarLocalidadePorLogradouro(logradouro);

			for (Localidade l : localidades) {
				if (l.getEstado() == estado && l.getCidade() == cidade && l.getBairro() == bairro
						&& l.getNumero() == numero)
					localidade = l;
			}
		} catch (Exception sqlException) {
